                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    {{-- <td><a style="border: 1px solid black; border-radius: 8px; padding: 5px;  color: black" href="{{route('chatFromVendor', ['user2' => 'Admin','user2_id'=> 2])}}">Open Conversation</a></td> --}}
                                    <td><a style="border: 1px solid black; border-radius: 8px; padding: 5px;  color: black" href="{{route('vendor.vendor.chat', ['user1_name' => Auth::guard('vendor')->user()->shopname,'user1_id'=> Auth::guard('vendor')->user()->id])}}">Open Conversation</a></td>
                                  </tr>
                              @foreach ($tickets as $item)
                              <tr>
                                <th scope="row">{{$loop->index+1}}</th>
                                <td>{{$item->user1_name}}</td>
                                <td>{{$item->status}}</td>
                                <td><small class="bg-dark text-light" style="border: 1px solid black; padding: 5px; border-radius: 5px;">{{isset($item->updated_at) ? date('d/M/y', strtotime($item->updated_at)): "0-0-0"}}</small></td>
                                <td><a style="border: 1px solid black; border-radius: 8px; padding: 5px;  color: black" href="{{route('vendorChatPage', ['ticket_id'=> $item->id])}}">Open Conversation</a></td>
                              </tr>
                              @endforeach
